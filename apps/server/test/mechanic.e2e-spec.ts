...
describe('Mechanic Dashboard (e2e)', () => {
    ...
    beforeAll(async () => {
        ...
        // Register Mechanic and Login to get Token
        await request(app.getHttpServer())
            .post('/auth/register')
            .send({ email: 'mech@test.com', password: 'Password123!', fullName: 'Mech One', role: 'Mechanic' });

        const mechRes = await request(app.getHttpServer())
            .post('/auth/login')
    ...
    it('MECH-002: Mechanic can access active jobs', () => {
        return request(app.getHttpServer())
            ...
            .expect((res) => {
                ...
                expect(res.body.length).toBe(5); // 5 mocked jobs
                expect(res.body[0].id).toBe('#JOB-1024');
            });
    });

    // MECH-003: Role Protection (Customer)
    ...
    // MECH-004: Role Protection (Unauth)
